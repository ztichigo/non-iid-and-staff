...
class DSGD(Optimizer):
    ...
    def __init__(self, params, model, update_period = 10, lr=required, momentum=0, dampening=0,
                 weight_decay=0, nesterov=False, local=None,args=None):
        ...
        with torch.no_grad():
            for group in self.param_groups:
                for p in group['params']:
                    ...
                    param_state["last_param_buff"] = p.clone().detach()
                    dist.all_reduce(param_state["last_param_buff"])
                    #dist.barrier()
                    param_state["last_param_buff"]/=size
                        
                    p.data.mul_(0).add_(param_state["last_param_buff"])
